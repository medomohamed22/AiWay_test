...
function selectTask(id){if(!isTaskVisible(id))return false;if((!userProfile||!userProfile.has_purchased)&&['coding','image'].includes(id)){toast(lang==='ar'?'هذه الأداة تتفعل بعد أول عملية شراء.':'This tool unlocks after your first purchase.');return false;}activeTask=id;storageSet('aiway_active_task',id);const modelId=chooseTaskModel(id);if(modelId&&$('model')){$('model').value=modelId;updateModelTrigger();syncModelMode()}current=null;history=[];closeTaskScreen();updateTaskContext();render();setTimeout(()=>{try{$('prompt')?.blur()}catch{}},50);return true}
...
window.AiWayActions={
  selectTask,
  newChat,
  openPackages:openPay,
  openSupport,
  toggleAuth:()=>auth?logout():login(),
  isAuthenticated:()=>Boolean(auth),
  setLanguage(next){
    if(!['ar','en'].includes(next)||next===lang)return false;
    lang=next;storageSet('aiway_lang',lang);sessionStorage.setItem('aiway_intro_lang',lang);applyLanguage();return true;
  }
};
