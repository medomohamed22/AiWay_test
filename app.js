...
function modelPriceLabel(m){if(m?.isAuto||m?.id==='openrouter/auto')return lang==='ar'?'اختيار تلقائي حسب طلبك':'Automatic selection for your request';const cost=modelCostValue(m);if(!Number.isFinite(cost))return '';if(m.type==='image')return lang==='ar'?`التكلفة تبدأ من $${cost.toFixed(cost<0.01?4:2)} للصورة`:`From $${cost.toFixed(cost<0.01?4:2)} per image`;return lang==='ar'?`التكلفة التقريبية: $${cost.toFixed(cost<1?3:2)} لكل مليون توكن`:`Approx. cost: $${cost.toFixed(cost<1?3:2)} per 1M tokens`}
...
function renderModelSelect(selected=''){
  ...
  const preferred=forcedTrial&&chatModels.some(m=>m.id==='openrouter/free')?'openrouter/free':(selected&&all.some(m=>m.id===selected&&!m.locked)?selected:((chatModels.find(m=>m.id==='openrouter/auto'&&!m.locked)||chatModels.find(m=>m.trial&&!m.locked)||chatModels.find(m=>!m.locked)||all.find(m=>!m.locked)||all[0]).id));
  ...
  renderModelMenu('free');updateModelTrigger();syncModelMode();
}
...
function renderModelMenu(filter='free'){
 const menu=$('modelMenu');if(!menu)return;
 const all=[...(window.aiwayModels||[]),...(window.aiwayImageModels||[])],orders=window.aiwayChatModelOrders||{};
 const auto=all.find(m=>m.id==='openrouter/auto'&&!m.locked),paidChat=all.filter(m=>m.type!=='image'&&!isFreeModel(m)&&m.id!=='openrouter/auto'),paidImages=all.filter(m=>m.type==='image'&&!m.locked);let items=[];
 const imageTask=activeTask==='image';
 ...
 if(filter==='paid')items=modelsByBackendOrder(paidChat,orders.paid,compareCostAsc);
 if(filter==='free')items=modelsByBackendOrder(all.filter(isFreeModel),orders.free,compareCostAsc);
 const labels={'image-cheap':lang==='ar'?'الأرخص':'Cheapest','image-expensive':lang==='ar'?'الأغلى':'Most expensive',cheap:lang==='ar'?'الأرخص':'Cheapest',paid:lang==='ar'?'المدفوع':'Paid',free:I18N[lang].modelsFree};
 ...
 const autoCard=!imageTask&&auto?`<button type="button" class="model-auto-option ${auto.id===$('model').value?'selected':''}" data-model-id="openrouter/auto"><span class="model-auto-spark">✦</span><span><b>AiWay Auto</b><small>${lang==='ar'?'AiWay تختار لك تلقائيًا — يعمل فعليًا عبر OpenRouter Auto':'AiWay chooses automatically — powered by OpenRouter Auto'}</small></span><i>${lang==='ar'?'موصى به':'Recommended'}</i></button>`:'';
 menu.innerHTML=autoCard+`<div class="model-tabs">${tabs}</div>`+items.map(m=>{const usage=m.type==='image'?{text:imageUnitCost(m)>0.08?(lang==='ar'?'استهلاك عالي جدًا للتوكين':'Very high token usage'):imageUnitCost(m)>0.03?(lang==='ar'?'استهلاك عالي للتوكين':'High token usage'):(lang==='ar'?'استهلاك منخفض للتوكين':'Low token usage'),level:imageUnitCost(m)>0.08?'very-high':imageUnitCost(m)>0.03?'high':'low'}:tokenUsageBadge(m),official=m.type==='image'?[]:officialModelBadges(m),badges=[...official,isFreeModel(m)?{text:lang==='ar'?'مجاني':'Free',level:'free'}:null,usage].filter(Boolean);const detail=m.providerLabel||m.familyLabel||m.provider||'AI';const price=modelPriceLabel(m);return `<button type="button" class="model-option ${m.id===$('model').value?'selected':''}" data-model-id="${esc(m.id)}" ${m.locked?'disabled':''}>${modelLogo(m)}<span class="model-option-copy"><b>${esc(shortModelName(m.shortName||m.name||m.id))}</b><small>${esc(detail)}${price?` · ${esc(price)}`:''}</small><span class="model-badges">${badges.map(x=>`<i class="model-badge ${x.level==='very-high'?'token-very-high':x.level==='high'?'token-high':x.level==='low'?'token-low':x.level==='smart'?'smart':x.level==='speed'?'speed':x.level==='coding'?'coding':x.level==='popular'?'popular':''}">${esc(x.text)}</i>`).join('')}</span></span><span class="model-lock">${m.locked?'🔒':''}</span><span class="model-option-check" aria-hidden="true"></span></button>`}).join('')||`<div class="model-menu-empty">${lang==='ar'?'لا توجد نماذج في هذه الفئة.':'No models in this category.'}</div>`;
 menu.querySelectorAll('[data-model-filter]').forEach(b=>b.onclick=e=>{e.stopPropagation();renderModelMenu(b.dataset.modelFilter)});menu.querySelectorAll('[data-model-id]').forEach(b=>b.onclick=()=>{selectModel(b.dataset.modelId);closeModelMenu()});
}
...
function openModelMenu(){const menu=$('modelMenu'),trigger=$('modelTrigger');if(!menu||!trigger)return;if(menu.parentElement!==document.body)document.body.appendChild(menu);try{renderModelMenu(activeTask==='image'?'image-cheap':'free')}catch(err){console.error('Model menu render failed',err);menu.innerHTML=`<div class="model-menu-empty">${lang==='ar'?'تعذر عرض النماذج. أعد المحاولة.':'Could not display models. Please try again.'}</div>`}menu.classList.add('model-menu-portal','open');const scrim=ensureModelMenuScrim();scrim.classList.add('open');document.body.classList.add('model-menu-open');trigger.classList.add('open');trigger.setAttribute('aria-expanded','true');positionModelMenu();requestAnimationFrame(()=>{positionModelMenu();menu.scrollTop=0})}
...
document.addEventListener('visibilitychange',()=>{if(auth){startSupportPolling();if(document.visibilityState==='visible')refreshMe().catch(()=>{})}});
...
const TASKS={
 ...
 'all-models':{ar:['AiWay PRO','اختر من نماذج الذكاء الاصطناعي المختلفة، أو دع AiWay Auto تختار الأنسب لك.','اختيار النموذج أو التوجيه التلقائي'],en:['AiWay PRO','Choose from different AI models, or let AiWay Auto select the best fit for you.','Manual or automatic model selection']}
};
...
function chooseTaskModel(id){if(userProfile&&!userProfile.has_purchased&&id!=='image')return 'openrouter/free';if(id==='image'){const list=(window.aiwayImageModels||[]).filter(m=>!m.locked&&!m.isFree).sort(compareCostAsc);return list[0]?.id||''}return id==='all-models'?'openrouter/auto':'aiway/auto'}
...
async function refreshMe(){const d=await api('/api/me');userProfile=d.user;window.aiwayUsageSummary=d.usageSummary||{};if(d.refreshedToken&&auth){auth={...auth,token:d.refreshedToken};storeAuthSession(auth)}renderAccountState();renderUsageSummary()}
...
const TASK_SUGGESTIONS={
 ...
 'all-models':{ar:[['اكتب محتوى','اكتب لي محتوى احترافي عن...'],['ساعدني في البرمجة','راجع هذا الكود واقترح أفضل حل:'],['حلل ملفًا','حلل الملف المرفق واستخرج أهم النقاط والقرارات.'],['اشرح موضوعًا','اشرح لي الموضوع التالي ببساطة وبأمثلة:']],en:[['Write content','Write professional content about...'],['Help with coding','Review this code and suggest the best solution:'],['Analyze a file','Analyze the attached file and extract key points and decisions.'],['Explain a topic','Explain the following topic simply with examples:']]}
};
...
async function continueResponse(index,automatic=false,autoDepth=0){
  if(streaming)return;
  ...
  if(!automatic&&!await aiwayConfirm(I18N[lang].continueConfirm,{title:lang==='ar'?'استكمال الرد؟':'Continue the response?',confirmText:lang==='ar'?'أكمل الرد':'Continue response',cancelText:lang==='ar'?'إلغاء':'Cancel',note:lang==='ar'?'سيُحسب الاستكمال كطلب جديد، وتُخصم التكلفة الفعلية فقط حسب النموذج والاستخدام.':'Continuation is a new request. Only its actual model usage will be charged.'}))return;
  let stageTimer=0;
  try{
    if(!auth){await login();return}
