...
function renderAccountState(){
  if(auth&&userProfile){
    ...
    $('loginBtn').querySelector('span:last-child').textContent=auth?I18N[lang].connected:(lang==='ar'?'تسجيل Pi Browser':'Sign in with Pi Browser');
  }
...
function toggleLanguage(){return setLanguage(lang==='en'?'ar':'en')}
...
function applyFeatureFlags(flags={}){window.aiwayFeatureFlags=flags||{};if(activeTask&&!isTaskVisible(activeTask)){activeTask='';storageRemove('aiway_active_task');updateTaskContext?.()}if($('taskScreen')?.classList.contains('open'))renderTaskScreen();for(const id of ['creditsButton','introHeroPackagesBtn','introHeroPackages']){const el=document.getElementById(id);if(el){el.hidden=flags.payments===false;el.setAttribute('aria-disabled',flags.payments===false?'true':'false')}}for(const id of ['topLoginBtn','introLoginBtn','introHeroLogin','mobileAccountLogin']){const el=document.getElementById(id);if(el&&flags.login===false){el.setAttribute('aria-disabled','true');el.classList.add('feature-disabled')}else el?.classList.remove('feature-disabled')}document.body.classList.toggle('maintenance-active',Boolean(flags.maintenance))}
...
function selectTask(id){
  if(!isTaskVisible(id))return false;
  if((!userProfile||!userProfile.has_purchased)&&['coding','image'].includes(id)){
    toast(lang==='ar'?'هذه الأداة تتفعل بعد أول عملية شراء.':'This tool unlocks after your first purchase.');
    return false;
  }
  activeTask=id;storageSet('aiway_active_task',id);
  const modelId=chooseTaskModel(id);
  if(modelId&&$('model')){$('model').value=modelId;updateModelTrigger();syncModelMode()}
  current=null;history=[];closeTaskScreen();updateTaskContext();render();
  setTimeout(()=>{try{$('prompt')?.blur()}catch{}},50);
  return true;
}
...
function setLoginBusy(busy){
 const ids=['loginBtn','topLoginBtn','introLoginBtn','introHeroLogin','mobileAccountLogin','continuePiSignin'];
...
function resetConversation(){current=null;history=[];closeMenu();render();return true}
async function newChat({openPicker=true}={}){resetConversation();if(openPicker)openTaskScreen();try{if(auth)await loadChats()}catch(e){toast(friendlyClientError(e,lang==='ar'?'تعذر تحديث قائمة الدردشات. يمكنك بدء المحادثة الآن.':'The chat list could not be refreshed. You can still start chatting.').message)}return true}
...
function openImageUrlPreview(src,alt=''){if(!src)return false;imagePreviewScale=1;imagePreviewX=0;imagePreviewY=0;$('imagePreviewImg').src=src;$('imagePreviewImg').alt=alt||(lang==='ar'?'معاينة الصورة المولدة':'Generated image preview');$('imagePreviewHint').textContent=lang==='ar'?'قرّب بإصبعين أو استخدم أزرار التكبير':'Pinch or use the zoom controls';applyImagePreviewTransform();$('imagePreviewModal').classList.add('open');$('imagePreviewModal').setAttribute('aria-hidden','false');return true}
async function loadRecentImages(limit=24){if(!auth)return [];const safeLimit=Math.min(48,Math.max(1,Number(limit)||24));const d=await api('/api/conversations?mode=images&limit='+safeLimit);return Array.isArray(d?.images)?d.images:[]}
...
window.AiWayApp=Object.freeze({
  selectTask,
  setLanguage,
  newChat,
  resetConversation,
  openConversation:openChat,
  openPayments:openPay,
  openSupport,
  signIn:login,
  signOut:logout,
  toggleAuth:()=>auth?logout():login(),
  openAttachmentPicker:()=>{$('fileInput')?.click();return true},
  toggleWebSearch:()=>{toggleWeb();return webSearch},
  toggleCostEstimate:()=>{setCostEstimateEnabled(!costEstimateEnabled());syncCostEstimateToggle();return costEstimateEnabled()},
  loadRecentImages,
  openImageUrlPreview,
  getLanguage:()=>lang,
