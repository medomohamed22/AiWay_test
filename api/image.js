...
async function getImageModels() {
  if (imageModelCache.models && Date.now() - imageModelCache.at < 3600000) return imageModelCache.models;
  ...
  imageModelCache = { at: Date.now(), models };
  return models;
}

async function getImageModel(preferQuality = false) {
  const models = await getImageModels();
  const primaryId = preferQuality ? QUALITY_IMAGE_MODEL_ID : FAST_IMAGE_MODEL_ID;
  const secondaryId = preferQuality ? FAST_IMAGE_MODEL_ID : QUALITY_IMAGE_MODEL_ID;
  const preferred = models.find(model => model.id === primaryId)
    || models.find(model => model.id === secondaryId);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const callImageProvider = requestBody => fetchWithTimeout('https://openrouter.ai/api/v1/images', {
      method: 'POST',
      headers: {
    ...
    let response = await callImageProvider(body);
    let payload = await response.json().catch(() => ({}));
    if (!response.ok && model.id === FAST_IMAGE_MODEL_ID) {
      const fallback = (await getImageModels()).find(item => item.id === QUALITY_IMAGE_MODEL_ID);
      if (fallback) {
        model = fallback;
        ({ requestBody: body, chosenResolution: selectedResolution, chosenAspectRatio: selectedAspectRatio } = buildImageRequest(model));
        estimatedCharge = estimateImageCharge(model, selectedResolution, hasReferenceImage);
        if (availableTokens < estimatedCharge.chargedTokens) throw appError('INSUFFICIENT_TOKENS_FOR_REQUEST', {
          availableTokens,
          requiredTokens: estimatedCharge.chargedTokens,
          shortfall: estimatedCharge.chargedTokens - availableTokens
        });
        response = await callImageProvider(body);
